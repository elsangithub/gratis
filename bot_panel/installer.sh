...
surat=$(curl -sS https://raw.githubusercontent.com/geovpn/perizinan/main/authbot | grep -w $ipes | awk '{print $4}'  )
if [[ "$surat" = "true" ]]; then
echo -ne
else
...
dircreate
        ...
        echo -ne "Username admin panel use '@' [Ex: @sampiiiiu] : "
        read admin_pnl
        [[ -z $admin_pnl ]] && admin_pnl="@sampiiiiu"
        echo ""
        echo -ne "Limit trial for reseller create user trial [default:1] : "
        read limit_pnl
        [[ -z $limit_pnl ]] && limit_pnl="1"
        echo ""
        echo -ne "Your name store [dafult: GEO-STORE] : "
        read store_pnl
        [[ -z $store_pnl ]] && store_pnl="GEO-STORE"
        echo ""
